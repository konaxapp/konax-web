...
export default function VistaCliente() {
  ...
  async function buscarClienteAutomatico(valorBusqueda) {
    ...
    const { data: cuentasData, error: errorCuentas } =
      await supabase
        ...
        cuentasData.forEach((cuentaEncontrada) => {
          const clienteEncontrado =
            clientesDeCuentas?.find(
              (item) =>
                String(item.id) ===
                String(cuentaEncontrada.cliente_id)
            );
  ...
  async function cambiarCuenta(cuentaId) {
    const nuevaCuenta = cuentas.find(
      (item) =>
        String(item.id) === String(cuentaId)
    );
  ...
  async function registrarPromesa() {
    ...
    if (!fechaPromesa || !montoPromesa) {
      alert(
        "Complete fecha y monto de promesa."
      );
      return;
    }
    ...
    if (
      ...
      alert(
        "El monto prometido no puede superar el saldo actual."
      );
      return;
    }
    ...
    const textoPromesa =
      `Promesa de pago para ${fechaPromesa} por ` +
      `${formatoDinero(monto)}. ` +
      `${observacionPromesa || ""}`;
    ...
    if (errorCobranza) {
      alert(
        "La promesa se guardó en la bitácora, " +
          "pero no se pudo actualizar cobranza: " +
    ...
    alert(
      "Promesa de pago registrada correctamente."
    );
  }
    ...
    if (!ventana) {
      alert(
        "El navegador bloqueó la ventana emergente. " +
          "Habilite ventanas emergentes para KONAX."
      );
      return;
    }
  ...
  function generarEstadoCuenta() {
    ...
    const historial =
      generarHistorialMovimientosEstadoCuenta();
            ...
            .map((mov, index) => {
              ...
              return `
                <tr style="background:${fondo};">
                  ...
                  <td class="ec-centro ${claseTipo}">
                    ${mov.tipo}
                  </td>

                  <td class="ec-derecha">
                    ${formatoDinero(mov.monto)}
                  </td>
    ...
    abrirDocumentoHTML(
      ...
      `
        <div class="ec-membrete">
          <div class="ec-negocio">
            ${nombreEmpresa()}
          </div>

          <div class="ec-subtitulo">
            ${subtituloEmpresa() || ""}
          </div>
        </div>
        ...
        <div class="ec-descripcion">
          Documento generado para consulta del saldo,
          pagos y recargos registrados.
        </div>

        ${
          datosContactoEmpresaLinea()
            ? `
              <div class="ec-contacto">
                ${datosContactoEmpresaLinea()}
              </div>
            `
            : ""
        ...
        <div class="pie">
          Estado de cuenta generado por
          ${nombreEmpresa()}
          mediante KONAX Gestión.
        </div>
      `
    );
  }
  ...
  function generarCartaMora() {
    ...
    if (diasAtraso <= 0) {
      const continuar = window.confirm(
        "Esta cuenta actualmente no presenta días de atraso. " +
          "¿Desea generar la carta de cobro de todas formas?"
      );
    ...
    abrirDocumentoHTML(
      ...
      `
        ...
        <p class="texto-carta">
          ...
          le comunica que la cuenta número
          <strong>
            ${cuenta.numero_cuenta || "-"}
          </strong>
        ...
        <p class="texto-carta">
          Le solicitamos realizar el pago
          correspondiente o comunicarse con el
          área de cobranza para regularizar el
          estado de su cuenta.
        </p>

        <p class="texto-carta">
          En caso de haber realizado el pago
          recientemente, agradecemos remitir el
          comprobante correspondiente para
          verificar y actualizar nuestros registros.
        </p>

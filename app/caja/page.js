...
export default function Caja() {
  ...
  const [fechaPago, setFechaPago] = useState(() => obtenerFechaPanama());
  ...
  async function crearCuentaComercialVentaCredito(
    ...
    const fechaVencimiento = new Date(`${fechaPago}T12:00:00`);
    fechaVencimiento.setMonth(fechaVencimiento.getMonth() + 1);

    const fechaVencimientoFinal = obtenerFechaPanama(fechaVencimiento);
      ...
      .insert([
        {
          ...
          responsable,
          codigo_producto:
            codigoProducto || productoSeleccionado?.codigo || null,
      ...
      .insert([
        {
          ...
          estado_cobranza:
            saldoInicial <= 0 ? "Cancelado" : "Al Día",
          fecha_ultimo_pago:
            esAbonoProducto() ? fechaPago : null,
          monto_ultimo_pago:
            esAbonoProducto() ? Number(monto || 0) : 0,
  ...
  async function actualizarSaldoCuenta(
    ...
    if (montoPago > saldoAnterior) {
      alert(
        `El pago de $${montoPago.toFixed(
          2
        )} supera el saldo pendiente de $${saldoAnterior.toFixed(2)}.`
      );
        ...
        .insert([
          {
            ...
            responsable_cobro: null,
            observacion_cobro:
              `Registro creado automáticamente desde Caja. ` +
              `Pago aplicado a la cuenta ${
  ...
  async function guardarMovimiento() {
    ...
    if (
      (esVentaCredito() || abonoNuevoConProducto) &&
      !productoSeleccionado
    ) {
      alert(
        "Seleccione un producto válido del inventario o escriba un código existente."
      );
      return;
    }
    ...
    if (
      ...
      alert(
        "El código escrito no existe en inventario. Seleccione un producto válido."
      );
      return;
    }
    ...
    if (
      ...
      alert(
        "Para un abono inicial debe seleccionar un producto del inventario."
      );
      return;
    }
    ...
    if (
      ...
      alert(
        "El abono debe ser menor que el valor total del producto. " +
          "Si paga completo use Venta Contado."
      );
      return;
    }
    ...
    try {
      ...
      if (error) {
        alert(
          "El saldo se actualizó, pero hubo error registrando " +
            "el movimiento en caja: " +
            error.message
        );
        return;
      }
      ...
      const debeDescontarInventario =
        esVentaContado() || abonoNuevoConProducto;
      ...
      alert(
        abonoExistente
          ? `Abono aplicado correctamente a la cuenta ${
              cuentaParaMovimiento?.numero_cuenta || ""
            }.`
          : esAbonoProducto()
  ...
  function limpiarFormulario() {
    const opciones =
      obtenerOpcionesMovimiento(tipoNegocioEmpresa);
  ...
  const fechaHoyPanama = obtenerFechaPanama();
  ...
  const movimientosHoy = movimientos.filter((mov) => {
    ...
    return fechaMovimiento === fechaHoyPanama;
  });
      ...
      <div style={contenedor}>
        <div style={header}>
          <div>
            ...
            <p style={subtitulo}>
              Registro de ventas, pagos, abonos,
              mensualidades y contratos.
            </p>

            <p style={negocioTexto}>
              Tipo de negocio:{" "}
              <strong>
                {tipoNegocioEmpresa || "General"}
              </strong>
            </p>
          </div>
            ...
            <div style={grid}>
              <Campo label="Nombre">
                <input
                  value={nombreContado}
                  onChange={(e) =>
                    setNombreContado(e.target.value)
                  }
                  style={inputStyle}
              ...
              <Campo label="Cédula">
                <input
                  value={cedulaContado}
                  onChange={(e) =>
                    setCedulaContado(e.target.value)
                  }
                  style={inputStyle}
                  placeholder="Cédula"
                />
              ...
              <Campo label="Dirección">
                <input
                  ...
                  onChange={(e) =>
                    setDireccionContado(e.target.value)
                  }
                  style={inputStyle}
              ...
              <Campo label="Teléfono">
                <input
                  ...
                  onChange={(e) =>
                    setTelefonoContado(e.target.value)
                  }
                  style={inputStyle}
              ...
              <div style={clienteBox}>
                ...
                {cuentasCliente.length > 0 && (
                  <Campo label="Seleccionar cuenta">
                    ...
                    >
                      <option value="">
                        Nueva venta / sin cuenta existente
                      </option>
        ...
        {esVentaConProducto() && (
          <div style={card}>
            ...
            {esVentaCredito() && (
              <p style={ayuda}>
                Venta Crédito crea cuenta por cobrar,
                pero NO descuenta inventario.
              </p>
            )}
            ...
            {esAbonoProducto() &&
              cuentaSeleccionada && (
                <p style={ayuda}>
                  Este abono se aplicará a una cuenta
                  existente. No descontará inventario
                  nuevamente.
                </p>
              )}
            ...
            {productoSeleccionado && (
              <div style={clienteBox}>
                ...
                <p>
                  Precio crédito: $
                  {Number(
                    productoSeleccionado.precio_credito ||
                      0
                  ).toFixed(2)}
                </p>
              </div>
